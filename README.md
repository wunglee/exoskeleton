...
**NSCA 外骨骼系统** —— 基于 [yudao-cloud](https://gitee.com/zhijiantianya/yudao-cloud) 精简版改造的平台化中台底座，专注提供认证、计费、支付、网关等通用能力。

> 原项目声明：现在、未来都不会有商业版本，所有代码全部开源！
>
> 本项目在 yudao-cloud 精简版基础上，按需集成支付系统、会员中心模块，并移除 BPM、ERP、CRM、MES、AI、IoT、商城、微信公众号、数据报表等业务模块，形成轻量级外骨骼平台。
>
> 可参考 [《迁移文档》](https://cloud.iocoder.cn/migrate-module/) 了解模块迁移机制。
...
* 【NSCA 外骨骼】：在精简版基础上，**保留** 系统功能、基础设施、支付系统、会员中心；**移除** 工作流程、商城系统、微信公众号、CRM、ERP、MES、AI 大模型、IoT 物联网、数据报表
...
* ~~工作流使用 Flowable~~（已移除 BPM 模块）
* 高效率开发，使用代码生成器可以一键生成 Java、Vue 前后端代码、SQL 脚本、接口文档，支持单表、树表、主子表
* 实时通信，采用 Spring WebSocket 实现，内置 Token 身份校验，支持 WebSocket 集群
* 集成微信小程序、企业微信、钉钉等三方登陆（已移除微信公众号模块），集成支付宝、微信等支付与退款
* 集成阿里云、腾讯云等短信渠道，集成 MinIO、阿里云、腾讯云、七牛云等云存储服务
* ~~集成报表设计器、大屏设计器~~（已移除 Report 模块）

## 🦴 NSCA 外骨骼改造说明

本项目基于 yudao-cloud 精简版进行定制化改造，目标是为上层业务系统提供**认证、计费、支付、会员**等通用平台能力。

### 保留模块

| 模块 | 说明 | 用途 |
|-----|------|------|
| `system` | 系统功能 | 用户、角色、菜单、部门、租户、SSO 等 |
| `infra` | 基础设施 | 代码生成、文件存储、定时任务、监控、日志等 |
| `pay` | 支付系统 | 支付宝、微信支付对接，支付/退款/转账订单管理 |
| `member` | 会员中心 | 会员用户、等级、积分、签到、标签、分组 |
...
mysql -u root -p ruoyi-vue-pro < sql/mysql/exoskeleton-pay-member.sql
```
...
* 通用模块（必选）：系统功能、基础设施
* 通用模块（可选）：支付系统、会员中心
* 业务系统（按需）：本项目已移除 ERP、CRM、MES、商城、微信公众号、AI、IoT、BPM、报表 等垂直业务模块，聚焦平台底座能力
...
> NSCA 外骨骼已移除：bpm、mall、erp、crm、mes、ai、iot、mp、report 模块
...
| [Flowable](https://github.com/flowable/flowable-engine)                                     | 工作流引擎（本项目已移除 BPM 模块）| 6.8.0      | [文档](https://doc.iocoder.cn/bpm/)                                   |
